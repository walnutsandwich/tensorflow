...
#前面是定义各种需要得到的参数，后面函数基本上就是填数据运行图来计算参数的过程
	...
	def getWeights(self):              #获取隐含层权重w1
			return self.sess.run(self.weights['w1'])

	def getBiases(self):				#获取隐含层偏置b1
...
def get_random_block_from_data(data,batch_size): #定义一个随机顺序获取一块block数据的函数，
	...
	return data[start_index:(start_index + batch_size)] #按顺序取一个的batch_size的数据
...
for epoch in range(training_epochs):
	...
	for i in range(total_batch):    #对于每一轮训练
		batch_xs = get_random_block_from_data(X_train,batch_size)#随机按顺序获取一块数据
		cost = autoencoder.partial_fit(batch_xs) #partial_fit是前面类里定义的函数，不断用新的数据在旧的模型基础上训练、更新模型
		avg_cost += cost/n_samples*batch_size #计算平均损失,先除再乘
